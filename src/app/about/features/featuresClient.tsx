...
export default function FeaturesClient() {
  ...
  const paragraph =
    "text-sm md:text-base text-muted-foreground text-justify leading-relaxed";
  const highlight =
  ...
  return (
    <>
      ...
      <main className="mx-auto w-full max-w-6xl space-y-8 pb-15 px-3 sm:px-4">
        {/* Header */}
        <BaseSection
          title={t("headerTitle")}
          ...
          actions={
            <div className="flex flex-col-reverse sm:flex-row items-end sm:items-center justify-end sm:justify-normal gap-2">
              ...
              <Link href="/">
                <Button className="px-6 text-sm font-semibold bg-[rgb(var(--color-primary))] text-white hover:opacity-90">
                  🚀 {t("cta.analyze")}
                </Button>
              </Link>
            </div>
        ...
        >
          <p className={`${paragraph} mt-2`}>{t("introTagline")}</p>
        ...
        <FeatureSection id="deps" title={t("deps.title")}>
          ...
          <RulesList
            items={[
              ...
              { rule: t("deps.rules.warnSameMajor"), status: "Warning" as const },
  ...
  "dependencies": {
    ...
    "react": "18.3.1"
          ...
          <div className="mt-4">
            <LabelWithTip
              label={<span className="text-sm font-medium">{t("deps.howLatest")}</span>}
              tip={<span className="max-w-xs block text-sm">{t("deps.intro")}</span>}
            />
          </div>
        </FeatureSection>
        ...
        <FeatureSection id="dead" title={t("dead.title")}>
          <ul className="list-disc pl-5 space-y-1 mb-3 text-sm md:text-base text-muted-foreground text-justify leading-relaxed">
...
const API_KEY = "AIzaSyD3Y4Z5xxxxxxxxxxxxxxxxxxxxxxxxxxxxx";`}
          ...
          <div className="mt-4">
            <LabelWithTip
              label={<span className="text-sm font-medium">{t("secrets.noMutationTip")}</span>}
              tip={<span className="max-w-xs block text-sm">{t("secrets.noMutationAnswer")}</span>}
            />
          </div>
        </FeatureSection>
        ...
        <FeatureSection id="env" title={t("env.title")}>
          <ul className="list-disc pl-5 text-sm md:text-base text-muted-foreground space-y-1 text-justify leading-relaxed">
        ...
        <FeatureSection id="activity" title={t("activity.title")}>
          <p className={paragraph}>{t("activity.intro")}</p>
        </FeatureSection>
        ...
        <FeatureSection id="readonly" title={t("readonly.title")}>
          <ul className="list-disc pl-5 text-sm md:text-base text-muted-foreground space-y-1 text-justify leading-relaxed">
            <li>
              <span className={highlight}>{t("readonly.points.rust")}</span>
            </li>
        ...
        <FeatureSection id="jira" title={t("jira.title")}>
          <p className={`${paragraph} mb-3`}>{t("jira.intro")}</p>
          ...
          <p className="text-sm text-yellow-600 mt-3">⚠️ {t("jira.warningJira")}</p>

          <div className="mt-4">
            <LabelWithTip
              label={<span className="text-sm font-medium">{t("jira.labelOneClick")}</span>}
              tip={<span className="max-w-xs block text-sm">{t("jira.tip")}</span>}
            />
          </div>
        </FeatureSection>
        ...
        <Card className="mt-6">
          <CardContent className="flex flex-col md:flex-row items-center justify-between gap-4">
            <div>
              <h3 className="text-base md:text-lg font-semibold">{t("headerTitle")}</h3>
